...
sampling = 'uniform' ; bins = 10 ; count = 50 ; N = 625 ; scaling = False if sampling == 'uniform' else True #both same uniform mass distribution
...
def combination_input(x):
    from itertools import combinations
    combs = []
    for i in range(len(x)):
        combs += list(combinations(x, i+1))
    return combs
...
def mlp(seed, plotting, inp, fol, sim, cat, dust, snap, redshift, cs, cs_mask, sampling, bins, count, N, scaling, h_nodes, activation, optimizer, dropout, perc_train, loss, epochs, batch_size, fluxes, colors):
    ...
    print(xcols_string)
    print(np.vstack((avg_cv_score, score, sdss_score)).T)
    ...
    del(eagle, sdss, nn, plot, plot_shap)
...
for comb in combs:
    ...
    mlp(seed, plotting, inp, fol, sim, cat, dust, snap, redshift, cs, cs_mask, sampling, bins, count, N, scaling, h_nodes, activation, optimizer, dropout, perc_train, loss, epochs, batch_size, fluxes, colors)
