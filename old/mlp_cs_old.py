...
#preprocessing
sampling = 'uniform' ; bins = 10 ; count = 100 ; N = 600 #uniform mass distribution
...
x_train, y_train, x_test, y_test = eagle.preprocess(colors, eagle_xcols, eagle_ycols, eagle_xtype, sampling, bins, count, N, scaling=False)

#sdss
sdss = SDSS(sdss_xcols, sdss_ycols, sdss_xtype, redshift)
sdss.preprocess(eagle, colors, sampling, int(N*0.2), scaling=False)
...
X_train, X_val, Y_train, Y_val = cross_validation(x_train, y_train, K, seed) #TODO
...
nn.MLP_model()
result = nn.model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs, verbose=0, validation_data=(x_test, y_test)) #train #TODO
...
#plot
if plotting:
    plot = PLOT_NN(eagle, nn, xnames, ynames, dataname='eagle', N=len(y_test), inp=inp, sampling=str(sampling), score=score)#[mse[19], r2[19]] #TODO
...
#plot
if plotting:
    plot = PLOT_NN(eagle, nn, xnames, ynames, dataname='sdss', N=len(sdss.y), inp=inp, sampling=str(sampling), score=sdss_score)#[mse[19], r2[19]]
